...
if(!s.includes("app.get('/api/admin/dashboard'")){
  ...
  const routes=[
    ...
    "  const body='<p>Hello '+esc(b.customer_name||'')+',</p><p>Here is your Afiléon Motorsport end-of-day booking receipt and event summary.</p><h2>Booking</h2><p><b>Reference:</b> '+esc(b.public_id)+'<br><b>Date:</b> '+esc(isoDate(b.booking_date))+'<br><b>Service:</b> '+esc(svc?.name||b.service_id)+'</p><h2>Vehicle</h2><p>'+esc(receipt.vehicle.details)+'</p><h2>Checks completed</h2>'+checkHtml+'<h2>Incident chronology</h2>'+incidentHtml+'<h2>Damage status</h2><p><b>'+esc(receipt.damage_status)+'</b></p><h2>Payments</h2><p><b>Total:</b> '+gbp(b.amount_total)+'<br><b>Paid:</b> '+gbp(b.amount_paid)+'<br><b>Outstanding balance:</b> '+gbp(b.balance_amount)+'<br><b>Balance status:</b> '+esc(b.balance_status||((Number(b.balance_amount||0)<=0)?'paid':'Not recorded'))+'</p><h2>Final handover</h2>'+(hand?'<p><b>'+esc(hand.title||'Final handover')+'</b><br>Status: '+esc(hand.status||'Not recorded')+'<br>Time: '+esc(fmt(hand.timestamp))+(hand.notes?'<br>'+esc(hand.notes):'')+(hand.administrator?'<br>Recorded by '+esc(hand.administrator):'')+'</p>':'<p>Final handover details have not been recorded.</p>')+'<p style=\"margin-top:22px;color:#9fb0c7\">This receipt summarises the operational record held against booking '+esc(b.public_id)+'. Please contact '+esc(EMAIL_REPLY_TO)+' if anything appears incorrect.</p>';",
    "  const text=['Afiléon Motorsport — End-of-Day Customer Receipt','Booking: '+b.public_id,'Date: '+isoDate(b.booking_date),'Customer: '+String(b.customer_name||''),'Vehicle: '+String(receipt.vehicle.details),'Checks completed: '+checks.length,'Incidents: '+incidents.length,'Damage status: '+receipt.damage_status,'Total: '+gbp(b.amount_total),'Paid: '+gbp(b.amount_paid),'Outstanding balance: '+gbp(b.balance_amount),'Final handover: '+(hand?[(hand.status||''),(hand.notes||'')].filter(Boolean).join(' — '):'Not recorded')].join('\\n');",
