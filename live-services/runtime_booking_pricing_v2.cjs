...
function replaceReq(from, to, label) {
  if (s.includes(to)) return;
  if (!s.includes(from)) throw new Error(`Booking pricing v2: missing ${label}`);
  s = s.replace(from, to);
}
...
if (!s.includes('const OPERATIONS_BASE =')) {
  ...
  replaceReq(marker, helpers, 'Kent travel helpers');
}
...
if (!s.includes('venues: Object.values(KENT_VENUES)')) {
  const configNeedle = "r.json({ services: SERVICES.map(publicService), season: rows[0] || null, rolling_months: 6, currency: 'GBP', licence_email: LICENCE_EMAIL, terms_version: TERMS_VERSION, terms_url: TERMS_URL, cancellation_note:";
  const configReplacement = "r.json({ services: SERVICES.map(publicService), venues: Object.values(KENT_VENUES), operations_base: OPERATIONS_BASE.name, min_lead_days: 1, season: rows[0] || null, rolling_months: 6, currency: 'GBP', licence_email: LICENCE_EMAIL, terms_version: TERMS_VERSION, terms_url: TERMS_URL, cancellation_note:";
  replaceReq(configNeedle, configReplacement, 'config travel metadata');
}

if (!s.includes("app.post('/api/travel-estimate'")) {
  replaceReq(
...
if (!s.includes("same_day_unavailable")) {
  replaceReq(
...
if (!s.includes('let travel = { ok:true')) {
  replaceReq(
...
console.log('Applied robust Kent travel pricing, venue selection and no-same-day booking patch.');
